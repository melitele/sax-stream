...
XmlNode.prototype.createSaxParser = function (options) {
  ...
  parser.onopentag = function(node) {
    ...
    else if (options.tag === node.name || Array.isArray(options.tag) && options.tag.indexOf(node.name) !== -1 && !currentTag) {
      record = {};
  ...
  parser.onclosetag = function(tag) {
    debug('Closed "%s"', tag);
    if (options.tag === tag || Array.isArray(options.tag) && options.tag.indexOf(tag) !== -1 && !record.parent) {
